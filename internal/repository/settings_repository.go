...
func (r *SettingsRepository) GetByKey(key string) (*model.Setting, error) {
	...
	err := database.DB.Where(&model.Setting{Key: key}).First(&setting).Error
	if err != nil {
		return nil, err
	}
	return &setting, nil
...
func (r *SettingsRepository) Update(setting *model.Setting) error {
	...
	err := database.DB.Where(&model.Setting{Key: setting.Key}).First(&existing).Error
...
func (r *SettingsRepository) GetByCategory(category string) ([]model.Setting, error) {
	...
	err := database.DB.Where(&model.Setting{Category: category}).Find(&settings).Error
	return settings, err
}
